...
{
  ROS_INFO("sto se dogada");
  if (down_found == 0)
  {
    down_hole[0] = msg->point.x;
    down_hole[1] = msg->point.y;
    down_hole[2] = msg->point.z;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_down =
      nh.subscribe<geometry_msgs::PointStamped>("/firefly/detectionDown/hole_central_point", 1, callbackDown);
  ros::Subscriber sub_front =
      nh.subscribe<geometry_msgs::PointStamped>("/firefly/detectionFront/hole_central_point", 1, callbackFront);
  ...
  // waypoints.push_back(Eigen::Vector3d(4.0, 0, 10.0));
  waypoints.push_back(Eigen::Vector3d(6.0, 0, 6.5));
  waypoints.push_back(Eigen::Vector3d(6.5, 0.5, 5));
  waypoints.push_back(Eigen::Vector3d(6, -0.7, 5.5));
  // waypoints.push_back(Eigen::Vector3d(6, 3, 10));
  // waypoints.push_back(Eigen::Vector3d(8, 0, 5));
